...
public class memberServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("verified".equals(action)) {
			...
			memSvc.getAll().stream().filter(_memVO -> member_status.equals(_memVO.getMember_status()))
					.forEach(_memVO -> {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("getOne_for_Update".equals(action)) {
			...
			try {
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/Front_end/mem/listAll.jsp");
				failureView.forward(req, res);
			}
...
//						byte[] member_picture = bao.toByteArray();
			...
			if (!errorMsgs.isEmpty()) {
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/Front_end/mem/update_mem_input.jsp");
				failureView.forward(req, res);
				return;
			}
			...
			/**************************** 2開始新增資料 ******************************/
			MemService memSvc = new MemService();
			memVO = memSvc.updateMem(member_no, member_account, member_password, member_name, member_nick, member_sex,
					member_birthday, member_address, member_telephone, member_email, member_picture,
					member_credit_number, member_back_verification, member_buildday, member_point, member_status);
		...
		if ("insert".equals(action)) {
			...
			try {
				...
				if (member_account == null || member_account.trim().length() == 0) {
					...
					errorMsgs.add("帳號姓名: 只能是英文、數字 ，且長度必須在2~10之間");
				}
				System.out.println("檢查點2");
				/*********** 會員密碼 *******************/
...
//						}
				...
				_memVO = memSvc.addMem(member_account, member_password, member_name, member_nick, member_sex,
						member_birthday, member_address, member_telephone, member_email, member_picture,
						member_credit_number, member_back_verification, member_buildday, member_point, member_status);
...
//					    String member_password = "您的密碼";
				...
				String messageText = "Hello! " + member_name + " 請謹記您的密碼: " + member_password + "\n" + " (已經啟用)" + "\n"
						+ "請點擊連結重新登入" + "http://" + req.getServerName() + ":" + req.getServerPort()
						+ req.getContextPath() + "/Front_end/mem/mem.do?action=verified&verification=" + member_status;
		...
		if ("delete".equals(action)) {
			...
			try {
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/Front_end/mem/listAllMem.jsp");
				failureView.forward(req, res);
			}
		...
		if ("registstart".equals(action)) {
			...
			if (registinformation != null) {
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/Front_end/mem/member_regestinfomation.jsp");
				failureView.forward(req, res);
			}
		...
		if ("getOne_For_Display_Back".equals(action)) {
			...
			if (member_account == null || member_account.trim().length() == 0) {
				...
				errorMsgs.add("帳號: 只能是英文、數字 ，且長度必須在2~10之間");
			}
			...
			if(!errorMsgs.isEmpty()) {
				req.setAttribute("memVO", memVO);
				RequestDispatcher failureView = req
						.getRequestDispatcher("/Back_end/mem/select_page.jsp");
				failureView.forward(req, res);
				return;
			}
